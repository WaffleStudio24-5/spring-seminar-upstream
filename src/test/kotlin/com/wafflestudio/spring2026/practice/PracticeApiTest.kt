...
class PracticeApiTest(
    ...
    fun `세미나에 학생과 모임을 만들고 참여할 수 있다`() {
        ...
        mockMvc.get("/practice/seminars/$seminarId/students").andExpect {
            status { isOk() }
        ...
        mockMvc.get("/practice/meetings/$meetingId").andExpect {
            status { isOk() }
            jsonPath("$.participantCount") { value(1) }
        }

        mockMvc.delete("/practice/meetings/$meetingId/participations/$studentId").andExpect {
            status { isNoContent() }
        }
        mockMvc.get("/practice/meetings/$meetingId").andExpect {
    ...
    fun `없는 세미나나 학생을 가리키면 404를 반환한다`() {
        mockMvc.get("/practice/seminars/999999").andExpect {
            status { isNotFound() }
            jsonPath("$.code") { value("SEMINAR_NOT_FOUND") }
        }
        mockMvc.get("/practice/students/999999").andExpect { status { isNotFound() } }
        mockMvc.get("/practice/meetings/999999").andExpect { status { isNotFound() } }

        mockMvc.post("/practice/students") {
    ...
    private fun createSeminar(title: String): Long =
        idOf(
            mockMvc.post("/practice/seminars") {
    ...
    private fun createMeeting(
        ...
        idOf(
            mockMvc.post("/practice/seminars/$seminarId/meetings") {
                json(
                    mapOf(
                        "title" to "1회차 모임",
                        "description" to "설명",
    ...
    private fun join(
        ...
        mockMvc.post("/practice/meetings/$meetingId/participations") {
